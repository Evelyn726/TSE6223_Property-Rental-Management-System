...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_property'])) {
    ...
    $update_query = "UPDATE property
                     SET property_name = ?, location = ?, property_type = ?, rental_price = ?, description = ?, availability_status = ?
                     WHERE property_id = ?";
    $update_stmt = mysqli_prepare($conn, $update_query);
    mysqli_stmt_bind_param($update_stmt, "sssdssi", $property_name, $location, $property_type, $rental_price, $description, $availability_status, $property_id);
...
$edit_query = "SELECT * FROM property WHERE property_id = ?";
$edit_stmt = mysqli_prepare($conn, $edit_query);
mysqli_stmt_bind_param($edit_stmt, "i", $property_id);
mysqli_stmt_execute($edit_stmt);
$edit_result = mysqli_stmt_get_result($edit_stmt);
$editProperty = mysqli_fetch_assoc($edit_result);
...
<head>
    ...
    <!-- Favicon -->
    <link rel="icon" type="image/png" href="../images/icon/admin-edit-property-icon.jpg">
                ...
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        <div class="row"><div class="col-lg-8">
                            <h3 class="title-5 m-b-35">Edit Property</h3>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form method="post" action="">
                                        ...
                                        <input type="hidden" name="property_id" value="<?= h($editProperty['property_id']) ?>">

                                        <div class="form-group">
                                            <label>Property Name</label>
                                            <input type="text" name="property_name" class="form-control" value="<?= h($editProperty['property_name']) ?>" required>
                                        </div>

                                        <div class="form-group">
                                            <label>Location</label>
                                            <input type="text" name="location" class="form-control" value="<?= h($editProperty['location']) ?>" required>
                                        </div>

                                        <div class="form-group">
                                            <label>Property Type</label>
                                            <input type="text" name="property_type" class="form-control" value="<?= h($editProperty['property_type']) ?>">
                                        </div>

                                        <div class="form-group">
                                            <label>Rental Price (RM)</label>
                                            <input type="number" step="0.01" name="rental_price" class="form-control" value="<?= h($editProperty['rental_price']) ?>" required>
                                        </div>

                                        <div class="form-group">
                                            <label>Description</label>
                                            <textarea name="description" class="form-control" rows="4"><?= h($editProperty['description']) ?></textarea>
                                        </div>

                                        <div class="form-group">
                                            <label>Availability Status</label>
                                            <select name="availability_status" class="form-control">
                                                <option value="Available" <?= ($editProperty['availability_status'] == 'Available') ? 'selected' : '' ?>>Available</option>
                                                <option value="Occupied" <?= ($editProperty['availability_status'] == 'Occupied') ? 'selected' : '' ?>>Occupied</option>
                                                <option value="Unavailable" <?= ($editProperty['availability_status'] == 'Unavailable') ? 'selected' : '' ?>>Unavailable</option>
                                            </select>
                                        </div>
